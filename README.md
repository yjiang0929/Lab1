...
The ALU you will implement is a subset of the standard MIPS ALU. The number of operations supported has been reduced, but otherwise we are following that standard.
...
**Submit this plan by Monday, October 1** by pushing `work_plan.txt` to GitHub.
...
### Test Benches ###
Construct a (set of) test bench(es) for your ALU and submodules as appropriate.  We recommend that you create the tests before you create the ALU. Your top-level test harness must be in `alu.t.v` and instantiate the ALU module from `alu.v`.
...
## Submission ##
Push your files to your team GitHub repository (the one listed on the team formation spreadsheet), and submit a link to your team repo on Canvas. You should have the following:

1. Work plan (due 10/1)
